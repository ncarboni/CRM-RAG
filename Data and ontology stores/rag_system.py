...
class FusekiRagSystem:
    ...
    def analyze_question_type(self, question):
        """
        Analyze the question to determine its type and specialized handling needs.
        ...
        location_patterns = ["where is", "located", "location", "where can i find", "situated"]
        ...
        # Temporal question patterns
        temporal_patterns = ["when was", "what year", "time period", "date", "century", "era"]
        ...
        # Iconographic question patterns
        iconographic_patterns = ["depict", "represent", "symbol", "attribute", "iconography"]
        for pattern in iconographic_patterns:
            if pattern in question_lower:
                return {
                    "type": "iconographic",
                    "confidence": 0.8
    ...
    def route_and_answer_question(self, question):
        """
        Analyze question type and route to specialized answering methods.
        ...
        # Route to specialized methods based on type
        if analysis["type"] == "location" and analysis.get("entity_name") and analysis["confidence"] >= 0.7:
            # Use specialized location answering
            entity_name = analysis["entity_name"]
            logger.info(f"Using specialized location answering for entity: {entity_name}")
            result = self.answer_location_question(entity_name)
            
            # Format for API response
            return {
                "answer": result["answer"],
                "sources": self._format_location_sources(result.get("locations", [])),
                "question_type": "location"
            }
        
        # For other question types or if confidence is low, use general method
    ...
    def _find_location_path(self, entity_uri):
        """
        Find location path for an entity.
        
        Args:
            entity_uri: URI of the entity
            
        Returns:
            Dictionary with location path information
        """
        # Initialize result
        result = {
            "success": True,
        ...
        try:
            ...
            if not results["results"]["bindings"]:
                # No direct location, try to see if the entity itself is a location
                query = f"""
                PREFIX crm: <http://www.cidoc-crm.org/cidoc-crm/>
                PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                
                ASK {{ <{entity_uri}> rdf:type crm:E53_Place }}
                """
                
                self.sparql.setQuery(query)
                is_place = self.sparql.query().convert()
                
                if not is_place.get("boolean", False):
                    return {
                        "success": False,
                        "error": "No location information found for this entity"
                    }
                
                # Entity is a place, so we use it as our starting point
                location_uri = entity_uri
                location_label = result["entity_label"]
            else:
                # Entity has a direct location
                location_uri = results["results"]["bindings"][0]["location"]["value"]
                location_label = results["results"]["bindings"][0].get("locationLabel", {"value": location_uri.split("/")[-1]})["value"]
                
                # Add direct location to path
                result["path"].append({
                    "source_uri": entity_uri,
                    "source_label": result["entity_label"],
                    "relationship": "P55_has_current_location",
                    "relationship_label": "is located at",
                    "target_uri": location_uri,
                    "target_label": location_label,
                    "level": 0
                })
            
            # Step 2: Traverse up the location hierarchy
            current_uri = location_uri
            current_label = location_label
            level = 1
            
            while level < 5:  # Maximum depth of 5
                query = f"""
                PREFIX crm: <http://www.cidoc-crm.org/cidoc-crm/>
                PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
                ...
                result["path"].append({
                    ...
                    "relationship": "P89_falls_within",
                    "relationship_label": "is within",
            ...
            logger.error(f"Error finding location path: {str(e)}")
            return {
                "success": False,
                "error": f"Error finding location path: {str(e)}"
            }

    def _find_temporal_path(self, entity_uri):
        """
        Find temporal information for an entity.
        ...
        # Find production and temporal information
        query = f"""
        PREFIX crm: <http://www.cidoc-crm.org/cidoc-crm/>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        
        SELECT ?production ?timespan ?date ?dateLabel WHERE {{
        ...
        try:
            ...
            if not results["results"]["bindings"]:
                return {
                    "success": False,
                    "error": "No temporal information found for this entity"
                }
            
            for result_row in results["results"]["bindings"]:
                # Add production information
                production_uri = result_row["production"]["value"]
                
                result["path"].append({
                    "source_uri": entity_uri,
                    "source_label": result["entity_label"],
                    "relationship": "P108i_was_produced_by",
                    "relationship_label": "was produced by",
                    "target_uri": production_uri,
                    "target_label": "Production Event",
                    "level": 0
                })
                
                # Add timespan if available
                if "timespan" in result_row:
                    timespan_uri = result_row["timespan"]["value"]
                    
                    result["path"].append({
                        "source_uri": production_uri,
                        "source_label": "Production Event",
                        "relationship": "P4_has_time-span",
                        "relationship_label": "occurred during",
                        "target_uri": timespan_uri,
                        "target_label": "Time Span",
                        "level": 1
                    })
                    
                    # Add date if available
                    if "date" in result_row:
                        date_uri = result_row["date"]["value"]
                        date_label = result_row.get("dateLabel", {"value": date_uri.split("/")[-1]})["value"]
                        
                        result["path"].append({
                            "source_uri": timespan_uri,
                            "source_label": "Time Span",
                            "relationship": "P82_at_some_time_within",
                            "relationship_label": "at some time within",
                            "target_uri": date_uri,
            ...
            logger.error(f"Error finding temporal path: {str(e)}")
            return {
                "success": False,
                "error": f"Error finding temporal path: {str(e)}"
            }

    def _find_iconographic_path(self, entity_uri):
        """
        Find iconographic information for an entity.
        
        Args:
            entity_uri: URI of the entity
            
        Returns:
            Dictionary with iconographic information
        """
        # Initialize result
        result = {
            "success": True,
            "entity_uri": entity_uri,
            "entity_label": self.get_entity_label(entity_uri),
            "path_type": "iconographic",
            "path": []
        }
        
        # Find representation and character information
        query = f"""
        PREFIX vir: <http://w3id.org/vir#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        ...
        SELECT ?representation ?representationLabel ?character ?characterLabel WHERE {{
            ...
            # Connect to our entity (could be the representation itself, an atom, or related)
            {{
                FILTER(?representation = <{entity_uri}>)
            }} UNION {{
                ?representation vir:K1i_is_denoted_by <{entity_uri}>
            }} UNION {{
                <{entity_uri}> vir:K1i_is_denoted_by ?representation
            }} UNION {{
                ?representation vir:K24_portray <{entity_uri}>
            }}
        }}
        """
        ...
        try:
            ...
            if not results["results"]["bindings"]:
                # Try a different approach - look for attributes
                query = f"""
                PREFIX vir: <http://w3id.org/vir#>
                PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
                
                SELECT ?representation ?representationLabel ?attribute ?attributeLabel WHERE {{
                    ?representation a vir:IC9_Representation ;
                                   rdfs:label ?representationLabel ;
                                   vir:K17_has_attribute ?attribute .
                    ?attribute rdfs:label ?attributeLabel .
                    
                    # Connect to our entity
                    {{
                        FILTER(?representation = <{entity_uri}>)
                    }} UNION {{
                        ?representation vir:K1i_is_denoted_by <{entity_uri}>
                ...
                if not results["results"]["bindings"]:
                    return {
                        "success": False,
                        "error": "No iconographic information found for this entity"
                    }
                
                # Process attributes
                for result_row in results["results"]["bindings"]:
                    representation_uri = result_row["representation"]["value"]
                    representation_label = result_row["representationLabel"]["value"]
                    attribute_uri = result_row["attribute"]["value"]
                    attribute_label = result_row["attributeLabel"]["value"]
                    
                    # Add representation information if not already added
                    if not any(p["source_uri"] == entity_uri and p["target_uri"] == representation_uri for p in result["path"]):
                        result["path"].append({
                            "source_uri": entity_uri,
                            "source_label": result["entity_label"],
                            "relationship": "is_connected_to",
                            "relationship_label": "is connected to",
                            "target_uri": representation_uri,
                            "target_label": representation_label,
                            "level": 0
                        })
                    
                    # Add attribute information
                    result["path"].append({
                        "source_uri": representation_uri,
                        "source_label": representation_label,
                        "relationship": "K17_has_attribute",
                        "relationship_label": "has attribute",
                        "target_uri": attribute_uri,
                        "target_label": attribute_label,
                        "level": 1
                    })
                
                return result
            
            # Process representations and characters
            for result_row in results["results"]["bindings"]:
                representation_uri = result_row["representation"]["value"]
                representation_label = result_row["representationLabel"]["value"]
                ...
                if "character" in result_row:
                    character_uri = result_row["character"]["value"]
                    character_label = result_row["characterLabel"]["value"]
                    
                    result["path"].append({
                        "source_uri": representation_uri,
                        "source_label": representation_label,
                        "relationship": "K24_portray",
                        "relationship_label": "portrays",
                        "target_uri": character_uri,
                        "target_label": character_label,
                        "level": 1
                    })
            ...
            logger.error(f"Error finding iconographic path: {str(e)}")
            return {
                "success": False,
                "error": f"Error finding iconographic path: {str(e)}"
            }
    ...
    def answer_location_question(self, entity_name):
        """
        Answer a location-specific question using graph traversal.
        
        Args:
            entity_name: Name of the entity to locate
            
        Returns:
            Dictionary with answer and location data
        """
        # First find the entity URI from the name
        query = f"""
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        ...
        try:
            ...
            if not results["results"]["bindings"]:
                return {
                    "answer": f"I couldn't find an entity named '{entity_name}' in the knowledge base.",
                    "locations": []
                }
            ...
            location_info = self.location_specific_graph_query(entity_uri)
            ...
            if location_info["locations"]:
                ...
                sorted_locations = sorted(location_info["locations"], key=lambda x: x["level"])
                
                for loc in sorted_locations:
                    locations_text.append(loc["label"])
                
                # Format the answer
                answer = f"{location_info['entity_label']} is located in {locations_text[0]}"
                
                if len(locations_text) > 1:
                    answer += f", which is within {', which is within '.join(locations_text[1:])}"
                
                # Add coordinates if available
                for loc in sorted_locations:
                    if loc["coordinates"]:
                        answer += f". {loc['label']} is located at coordinates: latitude {loc['coordinates']['latitude']}, longitude {loc['coordinates']['longitude']}"
                        break
                ...
                return {
                    "answer": f"I found {location_info['entity_label']}, but couldn't determine its location from the knowledge base.",
                    "locations": []
                }
        
        except Exception as e:
            logger.error(f"Error in location question answering: {str(e)}")
            return {
