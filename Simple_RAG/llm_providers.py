...
Updated with batch embedding support for better performance.
"""
...
class BaseLLMProvider(ABC):
    ...
    def get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        """Get embeddings for multiple texts (default: one by one)"""
...
class OpenAIProvider(BaseLLMProvider):
    ...
    def get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        """Get embeddings for multiple texts in batched API calls"""
        from langchain_openai import OpenAIEmbeddings
...
class AnthropicProvider(BaseLLMProvider):
    ...
    def get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        logger.info(f"Batch embedding {len(texts)} texts using OpenAI")
...
class R1Provider(BaseLLMProvider):
    ...
    def get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        """Get embeddings for multiple texts using R1 (Cohere) API"""
        import requests
        ...
        logger.info(f"Batch embedding {len(texts)} texts using Cohere")
        response = requests.post(endpoint, headers=headers, json=data)
        
        if response.status_code == 200:
            return response.json()["embeddings"]
        else:
            raise Exception(f"R1 batch embedding request failed: {response.text}")
...
class OllamaProvider(BaseLLMProvider):
    ...
    def get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.info(f"Batch embedding {len(texts)} texts using Ollama")
            return self._embeddings.embed_documents(texts)
        except ImportError:
            logger.error("langchain_community package not installed. Please install it with 'pip install langchain-community'")
            raise
